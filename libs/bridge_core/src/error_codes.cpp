...
ErrorInfo classify_common_error(const std::string& message, const std::string& fallback_code) {
  ...
  if (message == "target path required") return {"INVALID_PARAMS", message};
  if (message == "source and target paths are the same") return {"INVALID_PARAMS", message};
  if (message == "recursive required for directory copy") return {"INVALID_PARAMS", message};
  if (message == "rename target must stay in same directory") return {"INVALID_PARAMS", message};
  if (message == "cannot move/copy/rename workspace root") return {"INVALID_PARAMS", message};
  if (message == "cannot move/copy directory into its own subtree") return {"INVALID_PARAMS", message};
  ...
  if (contains(message, "session conflict:")) return {"SESSION_CONFLICT", message};
  if (contains(message, "session rebase required:")) return {"SESSION_REBASE_REQUIRED", message};
  if (message == "selector target not found") return {"SELECTOR_NOT_FOUND", message};
  if (message == "change not found") return {"SESSION_CHANGE_NOT_FOUND", message};
  if (message == "change_id required") return {"INVALID_PARAMS", message};
  if (message == "path not found in session") return {"SESSION_PATH_NOT_STAGED", message};
  if (message == "invalid selector occurrence") return {"INVALID_PARAMS", message};
